...
if (!document.location.href.includes('login')) {
    ...
    dhead.className = "yourheader"
    //dhead.style = 'position:fixed;top:0;z-index:2;width:100%;background-color:white;box-shadow:rgba(0, 0, 0, 0.19) 0px 5px,rgba(0, 0, 0, 0.1) 0px 10px'
    ...
    //dfoot.style = "font-family:Kanit;left:0;width:100%;text-align:right;padding:10px;z-index:0;background-color:#999999;color:black;font-size:0.75em"
    b.appendChild(dfoot)
    let footposn = ()=>{
		(b.getBoundingClientRect(0).height > screen.height) ? dfoot.style.position = 'relative' : dfoot.style.position = 'absolute';
		if (document.location.href.includes('dashboard_dashboard.php')) dfoot.style.position = 'relative'
		//dfoot.style.bottom = "-170px"
		dfoot.style.bottom = "-50px"
    }
    footposn()
    b.onresize = footposn
    ScreenOrientation.onchange = footposn
