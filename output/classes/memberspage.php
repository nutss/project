...
{	
	...
	function fillGridData() 
	{
		...
		while( $data ) {
			...
			$row["username"] = runner_htmlspecialchars( $formattedUsername );
				...
				$row["displayusername"] = runner_htmlspecialchars( $data["staffFullName"] );
			$row["displayusername_attrs"] = "id=\"cellDisplayName".runner_htmlspecialchars($userid)."\"";
			
			$this->users[ $userid ]["displayUserName"] = $data["staffFullName"];
				$row["emailuser"] = runner_htmlspecialchars( $data["staffEmail"] );
			$row["emailuser_attrs"] = "id=\"cellEmail".runner_htmlspecialchars($userid)."\"";
			
			$this->users[ $userid ]["emailUser"] = $data["staffEmail"];
	...
	function fillGroups()
	{
		...
		$sql = "select ". $grConnection->addFieldWrappers( "GroupID" ) .", "
			...
			." from ". $grConnection->addTableWrappers( "project_uggroups" ) 
		...
		foreach( $groups as $group => $state ) {
			if( $state == 1 ) {				
				...
				$dcDelete->filter = DataCondition::_And( array( 
					...
					DataCondition::FieldEquals( "UserName", $user, 0, $this->pSet->isCaseInsensitiveUsername() ? dsCASE_INSENSITIVE : dsCASE_STRICT ) 
				));
