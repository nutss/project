...
class RemindPasswordPage extends RunnerPage
{
	...
	{		
		...
		if( $this->remindMethod() == RPM_RESET )
		{
			...
			$this->connection->exec("update ".$this->connection->addTableWrappers("staffUsers")." set "
 ...
 //RPM_RESET
	...
	protected function getUserSearchingSQLData()
	{
		...
		{
			$fullcUserNameField = $this->connection->addTableWrappers( "staffUsers" ).".".$this->connection->addFieldWrappers( $this->usernameFiled );
			$fullcEmailField = $this->connection->addTableWrappers( "staffUsers" ).".".$this->connection->addFieldWrappers( $this->emailField );
			$fullcPasswordField = $this->connection->addTableWrappers( "staffUsers" ).".".$this->connection->addFieldWrappers( $this->passwordFiled );
		}
		...
		$selectClause.= ", ".$this->connection->addFieldWrappers("staffStatus");
		
		$strSQLData = array();
		$strSQLData["where"] = $sWhere;		
		$strSQLData["sql"] = $selectClause." from ".$this->connection->addTableWrappers("staffUsers")." where ".$sWhere;
