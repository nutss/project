...
class RegisterPage extends RunnerPage
{
	...
	protected function activateNewUser()
	{
		...
		if( $this->cipherer->isFieldEncrypted("staffUsername") )
			$strUsername = $this->cipherer->MakeDBValue("staffUsername", $username, "", true);
		else
			$strUsername = make_db_value("staffUsername", $username);

		$sql = "select ".$this->getFieldSQLDecrypt("staffPassword")
			." from ". $this->connection->addTableWrappers( "staffUsers" ) 
			." where ".
			$this->connection->comparisonSQL( 
				$this->getFieldSQLDecrypt("staffUsername"), 
				$strUsername,
		...
		$dbPassword = $this->cipherer->DecryptField( "staffPassword", $data[0] );
		...
		$sql = "update ". $this->connection->addTableWrappers( "staffUsers" )
			." set ". $this->connection->addFieldWrappers( "staffStatus" )."=1 "
			." where " . 
			$this->connection->comparisonSQL( 
				$this->getFieldSQLDecrypt("staffUsername"), 
				$strUsername,
	...
	{			
		return projectURL() . GetTableLink( "register" )
			."?a=activate&u=".rawurlencode( base64_encode($this->regValues["staffUsername"]) )
	...
	protected function sendUserRegisterMessage()
	{
		$data = array();		
		if( GetGlobalData("userRequireActivation") ) {
			$data["activateurl"] = $this->getActivateUrl();
		}
		...
		return RunnerPage::sendEmailByTemplate($strEmail, "userregister", $data);
	}
	...
	protected function sendAdminRegisterMessage()
	{
		...
		$strEmail = "chanaphon.l@gmail.com";
	...
	if( GetGlobalData("userRequireActivation") ) {
		...
		$this->strUsername = $values["staffUsername"];
		$this->strPassword = $values["staffPassword"];
		$this->strEmail = $values["staffEmail"];
		...
		$passwordHash = md5( $values["staffPassword"] );
		$originalpassword = $values["staffPassword"];

		$retval = DoInsertRecord("staffUsers", $values, $blobfields, $this);
		if( GetGlobalData("userRequireActivation") ) {
			$this->prepActivationCode = md5( $this->strUsername.$passwordHash );
		}
		
		$values["staffPassword"] = $originalpassword;
	...
	{
		if( $this->cipherer->isFieldEncrypted("staffUsername") )
			$sUsername = $this->cipherer->MakeDBValue("staffUsername", $strUsername, "", true);	
		else 
			$sUsername = add_db_quotes("staffUsername", $strUsername);
			
		$strSQL = "select count(*) from ". $this->connection->addTableWrappers( "staffUsers" ) 
			. " where " . 
			$this->connection->comparisonSQL( 
				$this->getFieldSQLDecrypt("staffUsername"), 
				$sUsername,
	...
	{
		if( $this->cipherer->isFieldEncrypted("staffEmail") )
			$sEmail = $this->cipherer->MakeDBValue("staffEmail", $strEmail, "", true);	
		else 
			$sEmail = add_db_quotes("staffEmail", $strEmail);
			
		$strSQL = "select count(*) from ". $this->connection->addTableWrappers( "staffUsers" ) 
			." where ".
			$this->connection->comparisonSQL( 
				$this->getFieldSQLDecrypt("staffEmail"), 
				$sEmail,
				true
			);
