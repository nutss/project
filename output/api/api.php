...
class API {
	...
	public static function login() {
		...
		if ( $authType == REST_APIKEY ) {
			...
			if( GetGlobalData("loginHardcoded", false) ) {
				if( GetGlobalData("APIkey", "") == $APIkey ) {
					Security::createUserSession( GetGlobalData("UserName") );
					return true;
				}
				return false;
			}
			...
			return Security::login( $userData[ GetUserNameField() ], $userData[ GetPasswordField() ], true, true );
		}		
