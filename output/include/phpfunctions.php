...
function fileCustomExpression($file, $data, $field, $ptype, $table )
{
				...
				if($table=="researchProjectProposal" && $field=="contractSignFile")
	{
		...
		return $value;
				...
				if($table=="researchDisburseConsider" && $field=="researchConsiderFile")
	{
		;
		return $value;
	}
				if($table=="researchAppointmentConsider" && $field=="researchConsiderFile")
	{
		;
		return $value;
	}
				if($table=="researchRenewalConsider" && $field=="researchConsiderFile")
	{
		;
		return $value;
	}
...
function GetDefaultValue($field, $ptype, $table="")
{
				...
				if($table=="staffUsers" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
				...
				if($table=="process" && $field=="entryUserName")
	{
		return $_SESSION["UserName"];
	}
				if($table=="process" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				...
				if($table=="researchType" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
				...
				if($table=="researchOperatingPeriod" && $field=="researchOperatingPeriodEndDate")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="researchOperatingPeriod" && $field=="entryUserName")
	{
		return $_SESSION["UserName"];
	}
				if($table=="researchOperatingPeriod" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="researchAppointment" && $field=="researchAppointmentDate")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="researchAppointment" && $field=="entryUserName")
	{
		return $_SESSION["UserName"];
	}
				if($table=="researchAppointment" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="researchDisburse" && $field=="researchOperatingPeriodEndDate")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="researchDisburse" && $field=="entryUserName")
	{
		return $_SESSION["UserName"];
	}
				if($table=="researchDisburse" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="researchRenewal" && $field=="entryUserName")
	{
		return $_SESSION["UserName"];
	}
				...
				if($table=="admin_users" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
				...
				if($table=="researchConsider" && $field=="entryUserName")
	{
		return $_SESSION["UserName"];
	}
				if($table=="researchConsider" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="researchScholarshipRegister" && $field=="researchRegisterStartDate")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="researchScholarshipRegister" && $field=="researchRegisterEndDate")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="researchScholarshipRegister" && $field=="entryUserName")
	{
		return $_SESSION["UserName"];
	}
				if($table=="researchScholarshipRegister" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				...
				if($table=="researchScholarshipProposal" && $field=="entryUserName")
	{
		return $_SESSION["UserName"];
				...
				if($table=="researchScholarshipProposal" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="researchScholarshipConsider" && $field=="processName")
	{
		return "รับสมัครขอรับทุน";
	}
				if($table=="researchScholarshipConsider" && $field=="entryUserName")
	{
		return $_SESSION["UserName"];
	}
				...
				if($table=="researchProjectProposal" && $field=="entryUserName")
	{
		return $_SESSION["UserName"];
				...
				if($table=="researchProjectProposal" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="researchDisburseConsider" && $field=="processName")
	{
		return "ขออนุมัติการเบิกจ่าย";
	}
				if($table=="researchDisburseConsider" && $field=="entryUserName")
	{
		return $_SESSION["UserName"];
	}
				if($table=="researchDisburseConsider" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="researchAppointmentConsider" && $field=="processName")
	{
		return "นัดหมายและติดตามงาน";
	}
				if($table=="researchAppointmentConsider" && $field=="entryUserName")
	{
		return $_SESSION["UserName"];
	}
				if($table=="researchAppointmentConsider" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="researchRenewalConsider" && $field=="processName")
	{
		return "ขอขยายระยะเวลาโครงการ";
	}
				if($table=="researchRenewalConsider" && $field=="entryUserName")
	{
		return $_SESSION["UserName"];
	}
				if($table=="researchRenewalConsider" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="researchPeriod" && $field=="researchOperatingPeriodEndDate")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="researchPeriod" && $field=="entryUserName")
	{
		return $_SESSION["UserName"];
	}
				if($table=="researchPeriod" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="researchProjectRegister" && $field=="researchRegisterStartDate")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="researchProjectRegister" && $field=="researchRegisterEndDate")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="researchProjectRegister" && $field=="entryUserName")
	{
		return $_SESSION["UserName"];
	}
