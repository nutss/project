...
class UserInfoPage extends EditPage {
	...
	protected function regenerateTotp() {
		...
		if( !$twofSettings["available"] || !$twofSettings["methods"]["totp"] ) {
	...
	function getDataSourceFilterCriteria( $ignoreFilterField = "" ) {
		global $cUserNameField;
		return DataCondition::FieldEquals( $cUserNameField, $this->getUserName() );
	}
	...
	public function getEditFormat( $fName ) {
		global $cUserNameField;
		if( $fName == $cUserNameField ) {
			return EDIT_FORMAT_READONLY;
		}
		return parent::getEditFormat( $fName );
	}
	...
	protected function loadTwofData() {
		...
		$twofData->secret = $data[ $twofSettings["totpField"] ];
	...
	protected function verify2fSettings( $twofData ) {
		...
		if( $twofData->enable && !$twofSettings["methods"][ $twofData->method ]) {
	...
	protected function save2fData( $twofData ) {
		...
		if( $twofData->enable ) {
			if( $twofData->method === "email" ) {
				...
				$dc->values[ $twofSettings["totpField"] ] = $twofData->secret;
			} 
		}
	...
	public function auditLog( $values ) {
		...
		$keys = array( $cUserNameField => Security::getUserName() );
		$values[ $cUserNameField ] = Security::getUserName();
		$audit->LogEdit( $cLoginTable, $values, Security::currentUserData(), $keys );
	}
