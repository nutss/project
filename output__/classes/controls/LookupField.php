...
//				$spacer = $this->horizontalLookup ? "&nbsp;&nbsp;" : "<br/>";
	...
	public function getAutoFillData( $linkFieldVal )
	{
		...
		foreach( $autoCompleteFields as $aData ) {
			$masterFieldName = $aData["masterF"];
			$masterFieldType = $this->pageObject->pSetEdit->getFieldType( $masterFieldName );
			if ( IsFloatType( $masterFieldType ) ) {
				$data[ $aData["lookupF"] ] = formatNumberForEdit( $data[ $aData["lookupF"] ] );
			}
		}
