...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="SummaryProject" && $field=="disbureAmounts")
	{
		$value = number_format($data["projectBudgetCost"] - $data["disbureAmounts"],2);
;
		return $value;
	}
				if($table=="SummaryProjectDelay" && $field=="disbureAmounts")
	{
		$value = number_format($data["projectBudgetCost"] - $data["disbureAmounts"],2);
;
		return $value;
	}
				if($table=="projectAssign" && $field=="disbureAmounts")
	{
		$value = number_format($data["projectBudgetCost"] - $data["disbureAmounts"],2);
;
		return $value;
...
{
	$value = "";
				if($table=="project" && $field=="projectFile")
	{
		;
		return $value;
	}
				if($table=="taskTrack" && $field=="trackAttFille")
	{
		;
		return $value;
	}
				if($table=="projectStatusApprove" && $field=="projectFile")
	{
		;
		return $value;
	}
				if($table=="taskDisburse" && $field=="disburseAttFille")
	{
		;
		return $value;
	}
				if($table=="taskTrackApprove" && $field=="trackAttFille")
	{
		;
		return $value;
	}
				if($table=="SummaryProject" && $field=="projectFile")
	{
		;
		return $value;
	}
				if($table=="SummaryProjectDelay" && $field=="projectFile")
	{
		;
		return $value;
	}
				if($table=="taskRenew" && $field=="renewAttFille")
	{
		;
		return $value;
	}
				if($table=="taskRenewApprove" && $field=="renewAttFille")
	{
		;
		return $value;
	}
				if($table=="projectStatusScreen" && $field=="projectFile")
	{
		;
		return $value;
	}
				if($table=="taskDisburseApprove" && $field=="disburseAttFille")
	{
		;
		return $value;
	}
				if($table=="taskRenewOinion" && $field=="renewAttFille")
	{
		;
		return $value;
	}
...
function GetLWWhere($field, $ptype, $table = "")
{
	...
	if(!$table)
		$table = $strTableName;
		if($table=="taskTrack" && $field=="progressStep")
	{
		$strWhere = "project.id='".$_SESSION[$strTableName."_masterkey1"]."'";
		return $strWhere;
	}
		if($table=="taskTrack" && $field=="contactName")
	{
		$strWhere = "project.id='".$_SESSION[$strTableName."_masterkey1"]."'";
		return $strWhere;
	}
		if($table=="taskDisburse" && $field=="progressStep")
	{
		$strWhere = "project.id='".$_SESSION[$strTableName."_masterkey1"]."'";
		return $strWhere;
	}
		if($table=="taskTrackApprove" && $field=="progressStep")
	{
		$strWhere = "project.id='".$_SESSION[$strTableName."_masterkey1"]."'";
		return $strWhere;
	}
		if($table=="taskTrackApprove" && $field=="contactName")
	{
		$strWhere = "project.id='".$_SESSION[$strTableName."_masterkey1"]."'";
		return $strWhere;
	}
		if($table=="taskDisburseApprove" && $field=="progressStep")
	{
		$strWhere = "project.id='".$_SESSION[$strTableName."_masterkey1"]."'";
		return $strWhere;
	}
...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="ProjectContact" && $field=="entryUserName")
	{
		return $_SESSION["UserName"];
	}
				if($table=="ProjectContact" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="ProjectStep" && $field=="entryUserName")
	{
		return $_SESSION["UserName"];
	}
				if($table=="ProjectStep" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="ProjectType" && $field=="entryUserName")
	{
		return $_SESSION["UserName"];
	}
				if($table=="ProjectType" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="project" && $field=="projectStart")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="project" && $field=="projectEnd")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="project" && $field=="projectBudgetCost")
	{
		return "0.00";
	}
				if($table=="project" && $field=="entryUserName")
	{
		return $_SESSION["UserName"];
	}
				if($table=="project" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="taskTrack" && $field=="trackDate")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="taskTrack" && $field=="entryUserName")
	{
		return $_SESSION["UserName"];
	}
				if($table=="taskTrack" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				...
				if($table=="admin_users" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
				...
				if($table=="projectStatusApprove" && $field=="projectStart")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="projectStatusApprove" && $field=="projectEnd")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="projectStatusApprove" && $field=="projectBudgetCost")
	{
		return "0.00";
	}
				if($table=="projectStatusApprove" && $field=="entryUserName")
	{
		return $_SESSION["UserName"];
	}
				if($table=="projectStatusApprove" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="taskDisburse" && $field=="trackDisburseDate")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="taskDisburse" && $field=="entryUserName")
	{
		return $_SESSION["UserName"];
	}
				if($table=="taskDisburse" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="taskTrackApprove" && $field=="trackDate")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="taskTrackApprove" && $field=="entryUserName")
	{
		return $_SESSION["UserName"];
	}
				if($table=="taskTrackApprove" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="SummaryProject" && $field=="projectStart")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="SummaryProject" && $field=="projectEnd")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="SummaryProject" && $field=="approveDate")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="SummaryProject" && $field=="projectBudgetCost")
	{
		return "0.00";
	}
				if($table=="SummaryProject" && $field=="entryUserName")
	{
		return $_SESSION["UserName"];
	}
				if($table=="SummaryProject" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="SummaryProjectDelay" && $field=="projectStart")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="SummaryProjectDelay" && $field=="projectEnd")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="SummaryProjectDelay" && $field=="approveDate")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="SummaryProjectDelay" && $field=="projectBudgetCost")
	{
		return "0.00";
	}
				if($table=="SummaryProjectDelay" && $field=="entryUserName")
	{
		return $_SESSION["UserName"];
	}
				if($table=="SummaryProjectDelay" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="SummaryTaskTrack" && $field=="entryUserName")
	{
		return $_SESSION["UserName"];
	}
				if($table=="SummaryTaskTrack" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="projectAssign" && $field=="projectStart")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="projectAssign" && $field=="projectEnd")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="projectAssign" && $field=="projectBudgetCost")
	{
		return "0.00";
	}
				if($table=="projectAssign" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="taskRenew" && $field=="renewDate")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="taskRenew" && $field=="entryUserName")
	{
		return $_SESSION["UserName"];
	}
				if($table=="taskRenew" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="taskRenewApprove" && $field=="renewDate")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="taskRenewApprove" && $field=="entryUserName")
	{
		return $_SESSION["UserName"];
	}
				if($table=="taskRenewApprove" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="projectStatusScreen" && $field=="projectStart")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="projectStatusScreen" && $field=="projectEnd")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="projectStatusScreen" && $field=="projectBudgetCost")
	{
		return "0.00";
	}
				if($table=="projectStatusScreen" && $field=="entryUserName")
	{
		return $_SESSION["UserName"];
	}
				if($table=="projectStatusScreen" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="taskDisburseApprove" && $field=="trackDisburseDate")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="taskDisburseApprove" && $field=="entryUserName")
	{
		return $_SESSION["UserName"];
	}
				if($table=="taskDisburseApprove" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="taskRenewOinion" && $field=="renewDate")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="taskRenewOinion" && $field=="entryUserName")
	{
		return $_SESSION["UserName"];
	}
				if($table=="taskRenewOinion" && $field=="entryTime")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
	return "";
        ...
        if (isset($call['args']))
        {
            ...
            foreach ($call['args'] as $arg)
            {
                ...
                if (is_array($arg))
                {
					...
					$arrStr = strlen($arrStr) < $paramMaxDisplay 
						?  $arrStr 
						: substr($arrStr, 0, $paramMaxDisplay).'...';
                ...
                {
                	$arg = @strlen($arg) < $paramMaxDisplay 
						?  $arg 
						: substr($arg, 0, $paramMaxDisplay).'...';
                    $args[] = $j.'.&nbsp;'. runner_htmlspecialchars( $arg ).';';
                }
            }
...
function runner_error_handler($errno, $errstr, $errfile, $errline)
{
	...
	$errinfo = array();
	$errinfo['errno'] = $errno;
	$errinfo['errstr'] = runner_htmlspecialchars( $errstr );
	...
	$sqlStr = isset($strLastSQL) ? runner_htmlspecialchars(substr($strLastSQL,0,20480)) : '';
